...
export default function WorkflowDiagram() {
  ...
  return (
    <div className="relative rounded-2xl border border-[#E7E5E4] dark:border-white/[0.06] bg-white/70 dark:bg-[#0D0E13]/40 p-4 sm:p-8">
      <style>{`
      ...
      <div className="flex items-center justify-center gap-3 flex-wrap">
        <button
          ...
          className={`rounded-md border px-3 py-1.5 font-mono text-[11px] uppercase tracking-[0.08em] transition-colors ${
            scenario.key === "legit"
              ...
              : "border-[#E7E5E4] dark:border-white/[0.08] text-[#8E8E93] dark:text-[#5A5C66] hover:text-[#0A0A0A] dark:hover:text-white"
          }`}
        >
          ● Legitimate request
        </button>
        <button
          ...
          className={`relative h-6 w-11 shrink-0 rounded-full border border-[#D6D3D1] dark:border-white/[0.1] transition-colors ${
      ...
      {/* Scenario banner */}
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#E7E5E4] dark:border-white/[0.06] bg-[#FAFAFA] dark:bg-[#0A0B10] px-4 py-3">
        <div className="min-w-0">
          <p className="text-[13.5px] font-medium text-[#0A0A0A] dark:text-white">{scenario.title}</p>
          <p className="mt-1 truncate font-mono text-[11.5px] text-[#8E8E93] dark:text-[#5A5C66]">
            {scenario.request.agent} <span className="text-[#3B82F6]">→</span>{" "}
            <span className="text-[#737373] dark:text-[#8B8D98]">{scenario.request.verb}</span> {scenario.request.target}
          </p>
        </div>
        <div
      ...
      <div className="mt-6 grid items-center gap-4 md:grid-cols-[minmax(150px,1fr)_minmax(40px,80px)_auto_minmax(40px,80px)_minmax(150px,1fr)]">
        {/* Left: your agents */}
        <div className={`rounded-xl border bg-[#FAFAFA] dark:bg-[#0A0B10] p-4 transition-colors ${flowing ? "border-[#3B82F6]/40" : "border-[#E7E5E4] dark:border-white/[0.08]"}`}>
          ...
          <ul className="space-y-1.5">
            {["LangChain / CrewAI", "MCP servers", "Chatbots", "Apps + scripts"].map((t) => (
              <li key={t} className="flex items-center gap-2 font-mono text-[11.5px] text-[#737373] dark:text-[#8B8D98]">
        ...
        <div className="relative hidden h-10 md:block">
          <div className={`lelu-dash absolute inset-x-0 top-1/2 h-[1.5px] ${flowing ? "text-[#3B82F6] lelu-dash-run" : "text-black/[0.15] dark:text-white/[0.12]"}`} />
          <span className="absolute inset-x-0 top-0 text-center font-mono text-[9px] uppercase tracking-[0.08em] text-[#8E8E93] dark:text-[#5A5C66]">SDK request</span>
        </div>

        {/* Center: Lelu */}
        <div className="rounded-xl border border-[#D6D3D1] dark:border-white/[0.1] bg-[#FAFAFA] dark:bg-[#0A0B10] p-4 shadow-[0_0_50px_rgba(37,99,235,0.08)]">
          <div className="mb-3 flex items-center gap-2">
            <svg width="14" height="15" viewBox="0 0 32 35" className="fill-[#0A0A0A] dark:fill-[#FAFAFA]"><rect x="0" y="0" width="26" height="9" rx="4.5"/><rect x="3" y="13" width="26" height="9" rx="4.5"/><rect x="6" y="26" width="26" height="9" rx="4.5"/></svg>
            <span className="text-[14px] font-semibold text-[#0A0A0A] dark:text-white">lelu</span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-2 rounded-lg border px-3 py-2.5 transition-all duration-300 ${
                    active
                      ? "border-[#2563EB]/60 bg-[#60A5FA]/10 text-[#0A0A0A] dark:text-white"
                      : "border-[#E7E5E4] dark:border-white/[0.07] text-[#737373] dark:text-[#8B8D98]"
                  }`}
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className={active ? "text-[#60A5FA]" : "text-[#8E8E93] dark:text-[#5A5C66]"}>
                    <path d={m.icon} />
                  </svg>
          ...
          <div
            className={`lelu-dash absolute inset-x-0 top-1/2 h-[1.5px] ${
              outcome
                ...
                : "text-black/[0.15] dark:text-white/[0.12]"
            }`}
          />
          {outcome && scenario.blocked && (
            <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#E5484D]/60 bg-[#FAFAFA] dark:bg-[#0A0B10] px-1.5 font-mono text-[10px] font-bold text-[#E5484D]">✕</span>
          )}
          <span className="absolute inset-x-0 top-0 text-center font-mono text-[9px] uppercase tracking-[0.08em] text-[#8E8E93] dark:text-[#5A5C66] whitespace-nowrap">
        ...
        <div
          className={`rounded-xl border bg-[#FAFAFA] dark:bg-[#0A0B10] p-4 transition-all duration-300 ${
            outcome && !scenario.blocked ? "border-[#30A46C]/40" : "border-[#E7E5E4] dark:border-white/[0.08]"
          } ${outcome && scenario.blocked ? "opacity-40" : "opacity-100"}`}
        >
          <p className="mb-2.5 font-mono text-[10.5px] font-bold uppercase tracking-[0.1em] text-[#737373] dark:text-[#8B8D98]">External services</p>
          <ul className="space-y-1.5">
            {["Databases", "Email + Slack", "Payments", "SaaS APIs"].map((t) => (
              <li key={t} className="flex items-center gap-2 font-mono text-[11.5px] text-[#737373] dark:text-[#8B8D98]">
                <span className="h-1 w-1 rounded-full bg-black/25 dark:bg-white/25 shrink-0" /> {t}
              </li>
            ))}
          </ul>
          <p className="mt-2.5 font-mono text-[9.5px] uppercase tracking-[0.08em] text-[#8E8E93] dark:text-[#5A5C66]">tools + APIs</p>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-[minmax(150px,1fr)_minmax(0,2fr)]">
        <div className="flex flex-col justify-end">
          <div className="mb-3 hidden md:flex items-center gap-2 pl-6">
            <div className="lelu-dash-v h-10 w-[1.5px] text-black/[0.15] dark:text-white/[0.12]" />
            <span className="font-mono text-[9px] uppercase tracking-[0.08em] text-[#8E8E93] dark:text-[#5A5C66]">human review</span>
          </div>
          <div className="rounded-xl border border-[#E7E5E4] dark:border-white/[0.08] bg-[#FAFAFA] dark:bg-[#0A0B10] px-4 py-3">
            <p className="font-mono text-[10.5px] font-bold uppercase tracking-[0.1em] text-[#F5A623]">Humans</p>
            <p className="mt-1.5 flex items-center gap-2 text-[12px] text-[#737373] dark:text-[#8B8D98]">
        ...
        {/* Event log */}
        <div className="rounded-xl border border-[#E7E5E4] dark:border-white/[0.06] bg-[#FAFAFA] dark:bg-[#0A0B10] p-4">
            ...
            {scenario.steps.map((s, i) => {
              ...
              return (
                <li
                  ...
                  } ${latest ? "text-[#60A5FA]" : "text-[#737373] dark:text-[#8B8D98]"}`}
                >
                  <span className="text-[#8E8E93] dark:text-[#5A5C66] w-3 shrink-0 text-right">{i + 1}</span>
                  {s.text}
                </li>
              );
            })}
