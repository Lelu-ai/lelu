...
function Kicker({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-3">
      <span className="inline-flex items-center gap-1.5 text-[12px] font-medium uppercase tracking-[0.08em] text-white font-mono">
        {children}
      </span>
    </div>
  );
...
function SectionHead({
  ...
  return (
    ...
    >
      ...
      <h2 className="text-[22px] font-semibold tracking-tight sm:text-[26px] lg:text-[30px] text-white leading-[1.2]">
        {title}
      </h2>
      <p className="mt-3 text-[15px] leading-relaxed text-[#8B8D98] max-w-xl mx-auto">{sub}</p>
    </motion.div>
  );
}
...
export default function HomePage() {
  ...
  return (
    <div className="dark min-h-screen bg-[#0A0B10] text-[#EDEEF0] antialiased">

      {/* ── NAV ─────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-[#0A0B10]/80 backdrop-blur-md">
        <nav className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
          <Link href="/" className="flex items-center gap-2.5">
            <LeluMark size={18} className="brightness-0 invert" />
            <span className="text-white font-bold text-[14px] tracking-[0.08em] uppercase">Lelu</span>
          </Link>
          <div className="hidden sm:flex items-center gap-6 text-[13px] text-[#8B8D98]">
            <Link href="/docs" className="hover:text-white transition-colors">Docs</Link>
            <Link href="/sandbox" className="hover:text-white transition-colors">Sandbox</Link>
            <Link href="/about" className="hover:text-white transition-colors">About</Link>
            <a href="https://github.com/lelu-ai/lelu" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">GitHub</a>
          </div>
          <div className="flex items-center gap-3">
            {user === null && (
              <Link
                href="/login"
                className="text-[13px] text-[#8B8D98] hover:text-white transition-colors"
              >
                Sign in
              </Link>
            )}
            {typeof user === "object" && user !== null && (
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-white text-[#0A0B10] text-[11px] font-bold flex items-center justify-center"
                  aria-label="User menu"
                >
                  {initials}
                </button>
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-[200px] bg-[#0D0E13] border border-white/[0.08] rounded-lg shadow-xl overflow-hidden z-50">
                    <div className="px-4 py-3 border-b border-white/[0.06]">
                      <p className="text-[13px] font-semibold text-white truncate">{user.name}</p>
                      <p className="text-[12px] text-[#8B8D98] truncate">{user.email}</p>
                    </div>
                    <div className="py-1">
                      {[
                        ...
                        <Link key={item.href} href={item.href} onClick={() => setDropdownOpen(false)}
                          className="block px-4 py-2 text-[13px] text-[#C9C9D2] hover:bg-white/[0.04] transition-colors">
                          {item.label}
                        </Link>
                      ))}
                    </div>
      ...
      <section className="relative overflow-hidden px-4 pt-16 sm:pt-24 pb-12" aria-label="Hero">
        ...
        <div className="relative mx-auto max-w-6xl text-center">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <Link
              href="/docs"
              className="inline-flex items-center gap-2 rounded-full border border-white/[0.1] bg-white/[0.03] px-3 py-1.5 text-[12px] text-[#8B8D98] hover:text-white transition-colors"
            >
          ...
          <motion.h1
            ...
            className="mx-auto mt-6 max-w-3xl text-[36px] sm:text-[52px] font-semibold tracking-tight text-white leading-[1.08]"
          >
          ...
          <motion.p
            ...
            className="mx-auto mt-5 max-w-xl text-[15px] sm:text-[16px] leading-relaxed text-[#8B8D98]"
          >
          ...
          >
            ...
            <Link
              href="/sandbox"
              className="rounded-md border border-white/[0.12] px-5 py-2.5 text-[14px] font-medium text-white hover:bg-white/[0.05] transition-colors"
            >
              Try the sandbox
            </Link>
          </motion.div>
          ...
          >
            <button
              onClick={copyNpx}
              className="group inline-flex items-center gap-3 rounded-lg border border-white/[0.08] bg-[#0D0E13] px-4 py-2.5 font-mono text-[13px] text-[#C9C9D2] hover:border-white/[0.16] transition-colors"
            >
              <span className="text-[#30A46C]">$</span> npx -y lelu-mcp start
              {npxCopied ? (
                <span className="text-[11px] text-emerald-400">copied</span>
              ) : (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#5A5C66] group-hover:text-white transition-colors"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg>
              )}
            </button>
          </motion.div>
      ...
      {/* ── TRUST STRIP ─────────────────────────────────────────────── */}
      <section className="relative border-y border-white/[0.06] bg-white/[0.02] py-4" aria-label="Trust signals">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-6 px-4 sm:gap-10 text-[#5A5C66]">
          {[
            ...
            <div key={t} className="flex items-center gap-2">
              {i > 0 && <span className="hidden sm:block h-3 w-px bg-white/[0.08] -ml-5" aria-hidden />}
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid gap-3 md:grid-cols-5">
            {STEPS.map((s, i) => (
              <motion.div
                ...
                className="relative rounded-xl border border-white/[0.06] bg-[#0D0E13]/60 p-5 hover:border-white/[0.12] transition-colors"
              >
                <p className="font-mono text-[11px] font-bold text-[#5A5C66] mb-3">{s.n}</p>
                <p className="text-[14px] font-semibold text-white leading-snug mb-2">{s.title}</p>
                <p className="text-[12.5px] leading-relaxed text-[#8B8D98]">{s.body}</p>
          ...
          >
            {DECISIONS.map((d) => (
              <div
                key={d.label}
                className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-[#0D0E13]/60 px-4 py-3.5"
              >
                <span className="h-2 w-2 rounded-full shrink-0" style={{ background: d.color }} />
                <span className="font-mono text-[13px] font-medium" style={{ color: d.color }}>{d.label}</span>
                <span className="text-[12px] text-[#5A5C66] ml-auto">{d.desc}</span>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ── SEE IT IN ACTION ────────────────────────────────────────── */}
      <section className="relative px-4 py-16 sm:py-24 border-t border-white/[0.06]" aria-label="See it in action">
        <div className="mx-auto max-w-6xl">
          <SectionHead
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-2 items-start">
            ...
            <motion.div
              ...
              className="rounded-2xl overflow-hidden border border-white/[0.06] bg-[#0D0E13]"
            >
              <div className="flex items-center justify-between px-3 py-2 bg-white/[0.03] border-b border-white/[0.06]">
                ...
                <button
                  onClick={copy}
                  className="flex items-center gap-1.5 text-[11px] text-[#8B8D98] hover:text-white transition-colors"
                >
                  {copied ? (
                    <>
              ...
              <pre className="p-5 text-[13px] font-mono text-[#C9C9D2] leading-relaxed overflow-x-auto min-h-[220px]">
                {CODE[codeTab]}
              </pre>
            </motion.div>
          </div>
          <div className="mt-8 text-center">
            <Link href="/sandbox" className="text-[13px] text-[#60A5FA] hover:text-white transition-colors">
      ...
      {/* ── INTEGRATIONS ────────────────────────────────────────────── */}
      <section className="relative py-16 sm:py-24 overflow-hidden border-t border-white/[0.06]" aria-label="Integrations">
        <div className="px-4">
          <SectionHead
            kicker="Integrations"
      ...
      <section className="relative px-4 py-10 sm:py-14" aria-label="Call to action">
        <div className="mx-auto max-w-6xl">
          <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0D0E13]/60 px-8 py-14 text-center sm:px-16">
            <div
            ...
            />
            <h2 className="relative text-[22px] font-semibold tracking-tight sm:text-[26px] text-white">
              Ready to give your agents guardrails?
            </h2>
            <p className="relative mt-3 text-[15px] text-[#8B8D98] max-w-md mx-auto">
            ...
            <div className="relative mt-7 flex flex-wrap items-center justify-center gap-3">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-md border border-white/[0.12] px-5 py-2.5 text-[14px] font-medium text-white hover:bg-white/[0.05] transition-colors"
              >
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────── */}
      <footer className="border-t border-white/[0.06] px-4 py-12" aria-label="Footer">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-10 sm:flex-row sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-[260px] text-[12.5px] leading-relaxed text-[#5A5C66]">
            ...
            <div className="grid grid-cols-2 gap-10 sm:grid-cols-3 text-[13px]">
              <div>
                <p className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.08em] text-[#5A5C66]">Product</p>
                <ul className="space-y-2 text-[#8B8D98]">
                  ...
                  <li><Link href="/audit" className="hover:text-white transition-colors">Audit Log</Link></li>
                </ul>
              </div>
              <div>
                <p className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.08em] text-[#5A5C66]">Company</p>
                <ul className="space-y-2 text-[#8B8D98]">
                  <li><Link href="/about" className="hover:text-white transition-colors">About</Link></li>
                  <li><a href="https://github.com/lelu-ai/lelu" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">GitHub</a></li>
                  <li><a href="https://github.com/lelu-ai/lelu/discussions" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">Discussions</a></li>
                </ul>
              </div>
              <div>
                <p className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.08em] text-[#5A5C66]">Social</p>
                <ul className="space-y-2 text-[#8B8D98]">
                  <li><a href="https://x.com/lelu_auth" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">X / Twitter</a></li>
                  <li><a href="https://github.com/lelu-ai/lelu" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">GitHub</a></li>
                </ul>
              </div>
            </div>
          </div>
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.06] pt-6 text-[12px] text-[#5A5C66]">
